...
def persist_lines(
    config, lines, table_cache=None, file_format_type: FileFormatTypes = None
) -> None:
        ...
        if t == "RECORD":
            ...
            if archive_load_files and stream in archive_load_files_data:
                ...
                if "column" in stream_archive_load_files_values:
                    incremental_key_column_name = stream_archive_load_files_values[
                        "column"
                    ]
            ...
            if row_count[stream] >= batch_size_rows:
                flush = True
                LOGGER.info(
                    "Flush triggered by batch_size_rows (%s) reached in %s",
                    batch_size_rows,
                    stream,
                )
                ...
                flush = True
                LOGGER.info(
                    "Flush triggered by batch_wait_limit_seconds (%s)",
            ...
            if stream not in schemas or schemas[stream] != new_schema:
                ...
                if archive_load_files:
                    ...
                    if incremental_key_column_name:
                        LOGGER.info(
                            "Using %s as incremental_key_column_name",
                            incremental_key_column_name,
                        )
                        archive_load_files_data[stream].update(
                            column=incremental_key_column_name, min=None, max=None
                        )
                    else:
                        LOGGER.warning(
                            "archive_load_files is enabled, but no incremental_key_column_name was found. "
                            "Min/max values will not be added to metadata for stream %s.",
                            stream,
                        )
...
def load_stream_batch(
    ...
    if row_count[stream] > 0:
        flush_records(
            stream, records, db_sync, temp_dir, no_compression, archive_load_files
        )
...
def flush_records(
    ...
    if archive_load_files:
        ...
        if (
            "schema_name" not in stream_name_parts
...
def main():
    ...
    if args.config:
        with open(args.config, encoding="utf8") as config_input:
            ...
            config.setdefault("private_key", os.getenv("TARGET_SNOWFLAKE_PRIVATE_KEY"))
            config.setdefault("password", os.getenv("TARGET_SNOWFLAKE_PASSWORD"))
            config.setdefault(
                "use_browser_authentication",
                os.getenv("TARGET_SNOWFLAKE_USE_BROWSER_AUTHENTICATION"),
            )
