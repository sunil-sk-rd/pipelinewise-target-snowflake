...
class TestDBSync(unittest.TestCase):
    ...
    def test_config_validation(self):
        # --- Robust authentication method tests ---
        # 1. Password only (already tested by minimal_config)
        ...
        # 2. Private key only
        config_private_key = minimal_config.copy()
        config_private_key.pop("password")
        config_private_key["private_key"] = "dummy-key"
        self.assertEqual(len(validator(config_private_key)), 0)

        # 3. Private key path only
        config_private_key_path = minimal_config.copy()
        config_private_key_path.pop("password")
        config_private_key_path["private_key_path"] = "/tmp/dummy-key.pem"
        self.assertEqual(len(validator(config_private_key_path)), 0)

        # 4. Browser authentication only
        config_browser_auth = minimal_config.copy()
        config_browser_auth.pop("password")
        config_browser_auth["use_browser_authentication"] = True
        self.assertEqual(len(validator(config_browser_auth)), 0)

        # 5. No authentication method (should fail)
        config_no_auth = minimal_config.copy()
        config_no_auth.pop("password")
        self.assertGreater(len(validator(config_no_auth)), 0)
    ...
    def test_create_query_tag(self):
        ...
        self.assertEqual(
            db_sync.create_query_tag("This is a test query tag"),
            "This is a test query tag",
        )
        self.assertEqual(
            db_sync.create_query_tag(
                "Loading into {{database}}.{{schema}}.{{table}}",
    ...
    def test_parallelism(self, query_patch):
        ...
        self.assertEqual(
            db_sync.DbSync(
                {**minimal_config, **external_stage_with_parallel}
            ).connection_config["parallelism"],
            5,
        )
        ...
        self.assertEqual(
            db_sync.DbSync(
                {**minimal_config, **table_stage_with_parallel}
            ).connection_config["parallelism"],
            5,
        )

    @patch(
        "target_snowflake.upload_clients.s3_upload_client.S3UploadClient.copy_object"
    )
    @patch("target_snowflake.db_sync.DbSync.query")
    ...
    def test_merge_failure_message(self, load_file_merge_patch, query_patch):
        ...
        with self.assertRaises(Exception), self.assertLogs(
            logger=LOGGER_NAME, level="ERROR"
        ) as captured_logs:
    ...
    def test_sync_table_with_no_changes_to_pk(self, query_patch):
        ...
        query_patch.side_effect = [[{"type": "CSV"}], [{"column_name": "ID"}], None]
    ...
    def test_sync_table_with_new_pk_in_stream(self, query_patch):
        ...
        self.assertEqual(
            'show primary keys in table dummy-db.dummy-schema."TABLE1";', calls[1][0][0]
        )

        self.assertEqual(
            'alter table dummy-schema."TABLE1" drop primary key;', calls[2][0][0][0]
        )
    ...
    def test_sync_table_with_stream_that_has_no_pk_but_get_a_new_one(self, query_patch):
        ...
        query_patch.side_effect = [[{"type": "CSV"}], [], None]
        ...
        query_patch.assert_has_calls(
            [
                ...
                call(
                    [
                        'alter table dummy-schema."TABLE1" add primary key("ID");',
                        'alter table dummy-schema."TABLE1" alter column "ID" drop not null;',
                    ]
                ),
            ]
